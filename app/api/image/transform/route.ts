...
function appendMetadataToFormData(
	...
	switch (method) {
		case "uncrop":
			...
			break;
		case "square":
			if (!metadata.targetRes) throw new Error("targetRes is required for square");
			formData.append("targetRes", metadata.targetRes);
			break;
		case "upscale":
			if (!metadata.factor) throw new Error("factor is required for upscale");
			formData.append("upscaleRatio", metadata.factor);
			break;
	}
}
	...
	try {
		...
		for (let i = 0; i < entries.length; i += 3) {
			...
			try {
				...
				console.error(`🚨 Failed to process ${metadata.originalFileName}:`, error);
				results.push({
					processId: "",
					originalFileName: metadata.originalFileName,
